...
ConnectionManager.prototype.initPools = function(sequelize) {
    ...
    } else if (config.database && config.database.indexOf('TNS=') >= 0) {
        connectionConfig.connectString = config.database.split('TNS=')[1];
    } else {
        connectionConfig.connectString = '//' + config.host + ':' + config.port + '/' + config.database;
    }

    if(_.isEmpty(config.pool)) {
        connectionConfig.poolMax = 20;
        connectionConfig.poolMin = 2;
        connectionConfig.poolTimeout = 60;
        connectionConfig.poolPingInterval = 30;
        connectionConfig.poolIncrement = 1;
        connectionConfig.queueRequests = true;
        connectionConfig.queueTimeout = 60;
    } else {
        connectionConfig.poolMax = config.pool.max;
        connectionConfig.poolMin = config.pool.min;
        connectionConfig.poolTimeout = config.pool.idle;
        connectionConfig.poolPingInterval = config.pool.pingInterval;
        connectionConfig.poolIncrement = config.pool.poolIncrement;
        connectionConfig.queueRequests = config.pool.queue;
        connectionConfig.queueTimeout = config.pool.queueTimeout;
    }
